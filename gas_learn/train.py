...
class Training:
    def train(self, file_path):
        ...
        raw_range = round(
            np.median(gas.range.iloc[len(gas) - 120:len(gas)]) +
            np.median(gas.range.iloc[len(gas) - 74:len(gas)]))
        raw_range /= 2
        rate_f = 0
        fee_range = 0
        if (raw_range <= 777):
        ...
        for i in range(14):
            ...
            for j in range(round(raw_ex[2] / 2), raw_ex[2] - raw_ex[1]):
                gas_train_ex.iloc[j, i] = gas_train_sort.iloc[
                    len(gas_train) - 1 - round(
                        (raw_ex[2] / 2 - 1 + raw_ex[1]) / 2 * len(gas_train))]
            for j in range(raw_ex[2] - raw_ex[1], raw_ex[2] - raw_ex[0]):
                gas_train_ex.iloc[j, i] = gas_train_sort.iloc[
                    len(gas_train) - 1 - round(((raw_ex[1] + raw_ex[0]) / 2) /
                                               raw_ex[2] * len(gas_train))]
        ...
        for i in range(len(fee_train)):
            fee_train_sort = fee_all.iloc[15001 - len(fee_train) + i -
                                          fee_range:15001 - len(fee_train) +
                                          i].copy().sort_values()
            ...
            gas_train_ex = pd.concat([
                ...
                fee_train.reset_index(drop=True)
                                     ...
                                     axis=1)
            gas_train = pd.concat([
                gas_train.reset_index(drop=True).iloc[:len(gas_train) -
                                                      120, :],
                gas_train_ex.reset_index(drop=True)
            ],
                                  axis=0)
