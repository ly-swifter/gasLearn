...
class Training:
    def train(self, file_path):
        ...
        print(raw_range)
        gas = gas.iloc[len(gas) - raw_range - 1000 : len(gas) - 120, :]
        fee = fee.iloc[len(fee) - raw_range - 1000 : len(fee) - 120]
        ...
        for i in range(len(fee_train)):
            if(fee_train_sort.iloc[15001 - len(fee_train) + i] == fee_train.iloc[i, 0])
              print('ok')
