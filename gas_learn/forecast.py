...
class Forecastting:
    def forecast(self, file_path, raw_range):
        ...
        forecast_d = 0
        _range_forecast = range_forecast.iloc[len(range_forecast) - 120: len(range_forecast), 3].copy()
                ...
                for i in range(len(range_forecast_t)):
                 ...
                 for i in range(len(range_forecast_t)):
                    if(range_forecast_t.iloc[i] > 0):
                        ...
                        range_forecast_t.iloc[i] = 1
        t = range_forecast_t.sum() / 30
        t = 1 / (1 + np.exp(-t)) - 0.5
        _range_forecast = _range_forecast * range_forecast_t
        forecast_d = 0
        for i in range(120):
            forecast_d =  forecast_d +  (120 - i) * _range_forecast.iloc[len(_range_forecast) - 1 - i] / 1800
        ...
        for i in range(119):
            ...
            forecast_m_t = forecast_m_t.iloc[1 : len(forecast_m_t)]
